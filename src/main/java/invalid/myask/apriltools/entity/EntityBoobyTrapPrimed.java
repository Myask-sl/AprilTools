...
public class EntityBoobyTrapPrimed extends EntityTNTPrimed {
    ...
    public void onUpdate() {
        super.onUpdate();
        if (toon && fuse == 0 && rand.nextFloat() < Config.anvil_drop_chance) {
            EntityAINearestAttackableTarget.Sorter findNearest = new EntityAINearestAttackableTarget.Sorter(this);
            ...
            if (!list.isEmpty()) {
                ...
                for (int i = 1; i < list.size(); i++)
                    if (findNearest.compare(cartoon_victim, list.get(i)) < 0) cartoon_victim = list.get(i);
                if (!worldObj.isRemote && worldObj.isAirBlock((int)cartoon_victim.posX, (int)cartoon_victim.posY + 2, (int)cartoon_victim.posZ)) {
                    worldObj.spawnEntityInWorld(new EntityFallingBlock(worldObj,
                        (int)cartoon_victim.posX + 0.5F, cartoon_victim.posY + 2, ((int)cartoon_victim.posZ) + 0.5F,
                        Blocks.anvil));
                }
            }
        }
